...
**Files:**
- Modify: `src-tauri/src/lib.rs` (setup: create audio backend + config + GlobalHotkeys + AppCore; install win32 overlay/tray/wheel on Windows; drain timers; `tauri::generate_context!` stays), `src-tauri/src/commands.rs`, `src-tauri/src/main.rs` (sole binary entry: no args -> `volumecontrol_tauri_lib::run()`; args present -> `volumectl::cli::run()`), `crates/volumectl/Cargo.toml` (remove the `[[bin]] name = "volumectl"` — volumectl becomes lib-only to avoid a cyclic package dependency), `src-tauri/Cargo.toml` (Windows-only deps: `windows-sys` features)
...
- Do NOT delete (deferred to Task 8, after the webview manual smoke): `crates/volumectl/src/mixer.rs`, `settings.rs`, `help.rs` — they stay as unused pub modules; keep `overlay.rs` + `tray.rs`.

**Architecture correction (controller, 2026-08-13):** the original plan said `crates/volumectl/src/main.rs` should "delegate to the Tauri host" — that creates a cyclic package dependency (volumectl pkg -> volumecontrol-tauri pkg -> volumectl pkg), which Cargo rejects. The binary moves INTO `src-tauri` (it already has `main.rs`); `crates/volumectl` becomes lib-only (remove its `[[bin]]`). CLI fallback (`get`/`set`/`mute`) is handled in the src-tauri `main.rs` by checking `std::env::args()` and calling `volumectl::cli::run()` (cli module exists, non-Windows) / no-op on Windows (Windows binary is GUI-only, as today).

**Interfaces:**
- Consumes: everything from Tasks 1–2b; existing `volumectl::app` (Windows) re-homed: the `AppContext`-equivalent logic now lives in `host_core::AppCore`; the old `app.rs` message loop is replaced by the Tauri event loop + the poll task from Task 2. Windows native surfaces (overlay/tray/wheel) are created in `native_win32.rs` and driven by AppCore's `EventSink`/action dispatcher.
- Produces: the production binary behaves exactly like today's app (hotkeys, HUD overlay, tray, wheel, blacklist, beep) plus the three webview surfaces; `src-tauri` is the single binary entry on all platforms (`volumecontrol_tauri_lib::run()`; CLI commands when args present).
...
- [ ] **Step 3: Rewire the binary entry** — remove `[[bin]] name = "volumectl"` from `crates/volumectl/Cargo.toml` (lib-only now; keep any other bins like `appkit_smoke`); `src-tauri/src/main.rs` becomes the sole binary: `fn main() { volumecontrol_tauri_lib::run() }` on all platforms, and when `std::env::args().nth(1).is_some()` (non-Windows) call `volumectl::cli::run()` for the CLI fallback (`get`/`set`/`mute`). The Linux/macOS headless host logic (`linux_host_core`/`macos_app`) is superseded by `AppCore` + Tauri host; keep the modules compiling (they stay `pub` in the volumectl lib).
