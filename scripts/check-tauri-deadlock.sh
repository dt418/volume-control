...
if rg -q 'custom-protocol' src-tauri/Cargo.toml; then
...
if rg -l 'WebviewWindowBuilder' src-tauri/src --glob '*.rs' --glob '!window_manager.rs' | grep -q .; then
...
for cmd in open_surface close_surface surface_ready; do
  if rg -q "pub async fn ${cmd}\(" src-tauri/src/commands.rs; then
...
if rg -q 'fn on_main' src-tauri/src/window_manager.rs; then
...
for wrapper in 'pub fn open(' 'pub fn close(' 'pub fn surface_ready('; do
  if rg -qF "${wrapper}" src-tauri/src/window_manager.rs; then
...
if rg -q 'run_on_main_thread' src-tauri/src/commands.rs &&
  rg -q 'async_runtime::channel' src-tauri/src/commands.rs; then
