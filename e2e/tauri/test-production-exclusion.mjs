...
const frontendPackage = await readFile(resolve(repositoryRoot, "frontend/package.json"), "utf8");
...
assert.notEqual(tauriConfig.app?.withGlobalTauri, true, "production config must not expose global Tauri APIs");
assert.doesNotMatch(frontendPackage, /@wdio\/tauri-plugin|@wdio\/tauri-service|webdriverio/);
